...
class CodeRunner(VGroup):
    def __init__(self, box_heights, codeMobject, **kwargs):
        if not all([isinstance(m, int) for m in box_heights]):
            raise Exception("All box_heights must be of type int")
        if not isinstance(codeMobject, Code):
            raise Exception("codeMobject not valid")
        VGroup.__init__(self, **kwargs)
        n = 0
        for h in box_heights:
            g = VGroup()
            g.add(codeMobject.code[n:n+h])
            n += h
            r = Rectangle(width=codeMobject.get_width()-0.5, height=g.get_height()+0.1,
                          color=YELLOW, fill_opacity=0, stroke_width=0).move_to(g).set_x(codeMobject.get_x())
            self.add(r)
    ...
    def runTo(self, lineIndex):
        if (lineIndex == -1 and self.currentLine != None):
            self.remove(self.currentLine)
            temp = FadeOutAndShift(self.currentLine, direction=LEFT)
            self.currentLine = None
            return temp
        if self.currentLine == None:
            self.currentLine = self[lineIndex].copy().set_opacity(0.2)
            self.add(self.currentLine)
            return FadeInFrom(self.currentLine, direction=LEFT)
        return Transform(self.currentLine, self[lineIndex].copy().set_opacity(0.2))
